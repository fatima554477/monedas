...
<img src="includes/contraer41.png" id="ocultar3" style="cursor:pointer;"/>&nbsp;&nbsp;&nbsp; TIPO DE CAMBIO&nbsp;<a style="color:red;font:12px">    (EUROS)</a></p></strong>
...
<div class="progress" style="width: 25%;">
        ...
        <div class="card">
          <div class="card-body">
                      ...
                      <form class="row g-3 needs-validation was-validated" id="MONEDASform2"  novalidate="" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
 
                      
                                 
<?php if($conexion->variablespermisos('','AGREGANUEVO_EURO','guardar')=='si'){ ?>
      ...
      <tr>
         ...
         <td><input type="text" class="form-control" id="validationCustom03" required=""    value="EURO" name="MONEDAS" readonly>
         </td>  </tr>
         <tr  style="background:#f7edf8"> 
         <th class="text-start"> <label for="validationCustom03" class="text-start">CÓDIGO ISO:</label></th>
         <td><input type="text" class="form-control" id="validationCustom03" required=""    value="EUR" name="ISO" readonly>
         </td>  </tr>
	...
	echo $encabezado1.$option2.'</select>';			
	?>
	</span>
...
<div id="drop_file_zone" ondrop="upload_file(event,'IMAGENEUROS')" ondragover="return false" >
<p>Suelta aquí o busca tu archivo</p>
<p><input class="form-control form-control-sm" id="IMAGENEUROS" type="text" onkeydown="return false" onclick="file_explorer('IMAGENEUROS');"  VALUE="<?php echo $IMAGEN_MONEDAS; ?>" required /></p>
<input type="file" name="IMAGENEUROS" id="nono"/>
<div id="1IMAGENEUROS">
<?php
if($IMAGEN_MONEDAS!=""){echo "<a target='_blank' href='includes/archivos/".$IMAGEN_MONEDAS."'>Visualizar!</a>"; 
}?></div>
</div>         
<div id="2IMAGENEUROS"><?php 
...
$querycontras21a33 = $monedas->Listado_fotosgUARDARMONEDAtemporal('IMAGENEUROS',$fecha,$sessionidem);

while($row2=mysqli_fetch_array($querycontras21a33)){
echo "<a target='_blank' href='includes/archivos/".$row2['IMAGENEUROS']."' id='A".$row2['id']."' >Visualizar!</a> "." <span id='".$row2['id']."' class='view_dataAEborrar' style='cursor:pointer;color:blue;'>Borrar!</span><span > ".$row2['fecha']."</span>".'<br/>';
}
?></div>			 
    ...
    <input type="hidden" value="hEUROS" name="hEUROS"/>     
 ...
 <button  style="float:right"  class="btn btn-sm btn-outline-success px-5"   type="button" id="GUARDAR_MONEDAS2">GUARDAR</button><div style="
    color: #f5f5f5;
...
id="mensajeMONEDAS2"/></th><?php } ?></tr></table>
           
            
                
 </form>

<?php if($conexion->variablespermisos('','AGREGANUEVO_EURO','email')=='si'){ ?>
           ...
           <?php
$querycontras = $monedas->Listado_EURO();
?>
...
<div style="position: relative; display: inline-block; margin-bottom: 10px;">
    ...
    <input type="text" id="buscadorMonedas22" placeholder="Buscar en tabla..." 
        class="form-control" 
...
<table class="table table-striped table-bordered" style="width:100%" id='reset_MONEDAS2' name='reset_MONEDAS2'>
<thead>
<tr style='background:#f5f9fc;text-align:center'>
    ...
    <?php if($conexion->variablespermisos('','AGREGANUEVO_EURO','modificar')=='si'){ ?>
        <th style="background:#c9e8e8">MODIFICAR</th>
    <?php } ?>
    <?php if($conexion->variablespermisos('','AGREGANUEVO_EURO','borrar')=='si'){ ?>
...
$urlIMAGEN_MONEDAS ='';
	...
	while($rowfotos = mysqli_fetch_array($urlIMAGEN_MONEDASid)){
		$urlIMAGEN_MONEDAS .= $conexion->descargararchivo($rowfotos["IMAGENEUROS"]);
	}
?>
<tr>
    ...
    <?php if($conexion->variablespermisos('','AGREGANUEVO_EURO','modificar')=='si'){ ?>
        <td><input type="button" name="view" value="MODIFICAR" id="<?php echo $row["id"]; ?>" class="btn btn-info btn-xs view_MONEDAS2" /></td>
    <?php } ?>
    <?php if($conexion->variablespermisos('','AGREGANUEVO_EURO','borrar')=='si'){ ?>
        <td><input type="button" name="view2" value="BORRAR" id="<?php echo $row["id"]; ?>" class="btn btn-info btn-xs view_dataMONEDASborrar2" /></td>
    <?php } ?>
</tr>
<?php } ?>
...
<script>
document.getElementById("buscadorMonedas22").addEventListener("keyup", function() {
    let input = this.value.toLowerCase();
    let filas = document.querySelectorAll("#reset_MONEDAS2 tbody tr");
