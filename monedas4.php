...
<img src="includes/contraer41.png" id="ocultar10" style="cursor:pointer;"/>&nbsp;&nbsp;&nbsp; TIPO DE CAMBIO&nbsp;<a style="color:red;font:12px">    (LIBRA ESTERLINA)</a></p></strong>


<div  id="mensajeMONEDAS42">
<div class="progress" style="width: 25%;">
        ...
        <div class="card">
          <div class="card-body">
                      ...
                      <form class="row g-3 needs-validation was-validated" id="MONEDASform4"  novalidate="" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
 
                      
                                 
<?php if($conexion->variablespermisos('','LIBRAE','guardar')=='si'){ ?>
      ...
      <tr>
         ...
         <th class="text-start" <label for="validationCustom03" class="form-label">MONEDA:</label></th>
         <td>
		 
		 <input type="text" class="form-control" id="validationCustom03" required=""    value="LIBRA ESTERLINA" name="MONEDAS" readonly>
         ...
         <tr  style="background:#f7edf8"> 
         <th class="text-start" <label for="validationCustom03" class="form-label">CÓDIGO ISO:</label></th>
         <td><input type="text" class="form-control" id="validationCustom03" required=""    value="GBP" name="ISO" readonly>
         </td>  </tr>
	...
	$encabezado2 = '<select class="form-select mb-3" aria-label="Default select example" id="BANCO_MONEDAS" required="" name="BANCO_MONEDAS">
	...
	$option2 .= '<option style="background: #'.$fondos[$num].'" '.$select.' value="'.$row1['banco'].'">'.$row1['banco'].'</option>';
	}
	echo $encabezado2.$option2.'</select>';			
	?>
	</span>
...
<div id="drop_file_zone" ondrop="upload_file(event,'IMAGENLIBRA')" ondragover="return false" >
<p>Suelta aquí o busca tu archivo</p>
<p><input class="form-control form-control-sm" id="IMAGENLIBRA" type="text" onkeydown="return false" onclick="file_explorer('IMAGENLIBRA');"  VALUE="<?php echo $IMAGEN_MONEDAS; ?>" required /></p>
<input type="file" name="IMAGENLIBRA" id="nono"/>
<div id="1IMAGENLIBRA">
<?php
if($IMAGEN_MONEDAS!=""){echo "<a target='_blank' href='includes/archivos/".$IMAGEN_MONEDAS."'>Visualizar!</a>"; 
}?></div>
</div>         
<div id="2IMAGENLIBRA"><?php 
...
$querycontras21a33 = $monedas->Listado_fotosgUARDARMONEDAtemporal('IMAGENLIBRA',$fecha,$sessionidem);

while($row2=mysqli_fetch_array($querycontras21a33)){
echo "<a target='_blank' href='includes/archivos/".$row2['IMAGENLIBRA']."' id='A".$row2['id']."' >Visualizar!</a> "." <span id='".$row2['id']."' class='view_dataAEborrar' style='cursor:pointer;color:blue;'>Borrar!</span><span > ".$row2['fecha']."</span>".'<br/>';
}
?></div>			 
    ...
    <input type="hidden" value="hMONEDAS4" name="hMONEDAS4"/>     
 ...
 <button  style="float:right"  class="btn btn-sm btn-outline-success px-5"   type="button" id="GUARDAR_MONEDAS4">GUARDAR</button><div style="
    color: #f5f5f5;
...
id="mensajeMONEDAS4"/></th> <?php } ?></tr></table>
           
            
                
 </form>

<?php if($conexion->variablespermisos('','LIBRAE','email')=='si'){ ?>

                  <form name="form_emil_MONEDAS4" id="form_emil_MONEDAS4">
				  <tr>
             <td ><textarea  placeholder="ESCRIBE AQUÍ TUS CORREOS SEPARADOS POR PUNTO Y COMA EJEMPLO: NOMBRE@CORREO.ES;NOMBRE@CORREO.ES" style="width: 500px;" name="EMAIL_MONEDAS4" id="EMAIL_MONEDAS4" class="form-control" aria-label="With textarea"><?php echo $EMAIL_MONEDAS4; ?></textarea>
            <button class="btn btn-sm btn-outline-success px-5"  type="button" id="BUTTON_MONEDAS4">ENVIAR POR EMAIL</button></td>   <?php } ?>
                
           </tr></table>


           <?php
$querycontras = $monedas->Listado_MONEDA4();
...
<div style="position: relative; display: inline-block; margin-bottom: 10px;">
    ...
    <input type="text" id="buscadorMonedas4" placeholder="Buscar en tabla..." 
        class="form-control" 
...
<table class="table table-striped table-bordered" style="width:100%" id='reset_MONEDAS4' name='reset_MONEDAS4'>
<thead>
<tr style='background:#f5f9fc;text-align:center'>
    ...
    <?php if($conexion->variablespermisos('','LIBRAE','modificar')=='si'){ ?>
        <th style="background:#c9e8e8">MODIFICAR</th>
    <?php } ?>
    <?php if($conexion->variablespermisos('','LIBRAE','borrar')=='si'){ ?>
...
while($row = mysqli_fetch_array($querycontras)) {	
 $urlIMAGEN_MONEDAS ='';
	...
	while($rowfotos = mysqli_fetch_array($urlIMAGEN_MONEDASid)){
		$urlIMAGEN_MONEDAS .= $conexion->descargararchivo($rowfotos["IMAGENLIBRA"]);
	}
?>
<tr>
    ...
    <?php if($conexion->variablespermisos('','LIBRAE','modificar')=='si'){ ?>
        <td><input type="button" name="view" value="MODIFICAR" id="<?php echo $row["id"]; ?>" class="btn btn-info btn-xs view_MONEDAS4" /></td>
    <?php } ?>
    <?php if($conexion->variablespermisos('','LIBRAE','borrar')=='si'){ ?>
        <td><input type="button" name="view2" value="BORRAR" id="<?php echo $row["id"]; ?>" class="btn btn-info btn-xs view_dataMONEDASborrar4" /></td>
    <?php } ?>
</tr>
<?php } ?>
</tbody>
...
<script>
document.getElementById("buscadorMonedas4").addEventListener("keyup", function() {
    let input = this.value.toLowerCase();
    let filas = document.querySelectorAll("#reset_MONEDAS4 tbody tr");
